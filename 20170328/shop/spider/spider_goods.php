...
function goodsPage($cat_id, $page) {
    ...
    //品牌编号
    $reg = '/<a class="who" href="\/i\/brand\/intro\/\?id=(.*?)" target="_blank"><img src=".*?">.*?<\/a>/s';
    $brandData = fetchData($content, $reg);
    $imgData[] = $brandData[1];

    //点赞数
    $reg = '/goods_uid="\d+">(.*?)<\/a>/m';
    $start_number = fetchData($content, $reg);
    $imgData[] = $start_number[1];
    ...
    $sql = createSQL($imgData, 'ucshop_goods', array('goods_id', 'goods_thumb', 'goods_name', 'price', 'goods_desc', 'brand_id', 'star_number', 'cat_id', 'is_hot'));
