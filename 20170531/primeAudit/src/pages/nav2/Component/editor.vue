	        ...
	        <div class="modal-dialog">
	            <div class="modal-content">
	                <div class="modal-header">
	                    ...
	                    <h4 class="modal-title" id="myModalLabel">项目管理 > 已激活 > 新增 > 编制人选择</h4>
	                </div>
	                  	...
	                  	<div style="height:100%">
		                	<table id="customerTable" cellspacing="0" width="100%" class="table table-striped" >
		                        ...
		                        <tbody class="cutTaBody">
		                          <template v-for="(item,key) in leadingList">
		                            <tr v-on:click="chooseList(item.Id,item.Name)" >
		                              ...
		                              <td>{{item.CompanyRanksId.Name}}</td>
		                              <td>{{item.StaffType.text}}</td>
		                            </tr>
		                          </template>
...
import qs from 'qs'
	export default{
		...
		methods:{
			close () {
		      	this.$emit('close');
		      	console.log(this.leadingList)
	    	},
	    	chooseList(editorId,editorName) {
	    		if(this.editorName === ""){this.editorName = editorName}
	    		else{this.editorName = this.editorName + "," + editorName}
	    		if(this.editorId === ""){this.editorId = editorId}
	    		else{this.editorId = this.editorId + "," + editorId}
	    	},
	    	confirm() {
	    		this.$emit("editorName",this.editorName)
	    		this.$emit("editorId",this.editorId)
	    		this.$emit('close')
	    		this.editorId = this.editorName = ""
	    	}
		},
		beforeMount() {
			var self = this
		    ...
		    .then(function (response) {
		    	self.leadingList = JSON.parse(response.data.data)
		    	self.leadingList = self.leadingList.Data
		    })
