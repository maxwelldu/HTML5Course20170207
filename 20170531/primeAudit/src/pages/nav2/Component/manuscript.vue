	        ...
	        <div class="modal-dialog">
	            <div class="modal-content">
	                <div class="modal-header">
	                    ...
	                    <h4 class="modal-title" id="myModalLabel">项目管理 > 已激活 > 查看 > 批量设置 > 底稿模板</h4>
	                </div>
	                  	...
	                  	<div style="height:100%">
		                	<table id="customerTable" cellspacing="0" width="100%" class="table table-striped" >
		                        ...
		                        <tbody class="cutTaBody">
		                          <template v-for="(item,key) in manuscriptList">
		                            <tr>
		                              <td>{{key+1}}</td>
		                              <td>{{item.Code}}</td>
		                              <td>{{item.Name}}</td>
		                              <td>{{item.AttachmentId.Name}}</td>
...
export default{
	...
	beforeMount() {
	    ...
	    var getCodePars = {
	    	'limit': "5",
	    	'page': "1",
	    	'p_schema': "BusinessType/ManuscriptTypeSelector",
	    	'start': "0",
	    	'filter': "[]",
	    	'isSelect': "false",
	    	'sort': "[]"
	    ...
	    .then(function (response) {
	    	self.manuscriptList = JSON.parse(response.data.data)
	    	self.manuscriptList = self.manuscriptList.Data
	    })
